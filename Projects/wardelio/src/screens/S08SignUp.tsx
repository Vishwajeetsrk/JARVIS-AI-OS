...
interface S08SignUpProps {
  onSignUp: () => void;
  onLogin: () => void;
  onBack: () => void;
}

export function S08SignUp({ onSignUp, onLogin, onBack }: S08SignUpProps) {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showPw, setShowPw] = useState(false);
  const [agreed, setAgreed] = useState(false);

  const valid = email.includes("@") && password.length >= 8 && agreed;
  ...
  return (
    <div className="screen" style={{
      ...
      <div style={{ flex: 1, padding: "8px 24px", display: "flex", flexDirection: "column" }}>
        ...
        }}>Start building your smart wardrobe today.</p>
        ...
        <label style={{
          ...
          fontSize: 13, color: tokens.colors.textSecondary, marginBottom: 24, cursor: "pointer",
        }}>
          <input
            type="checkbox"
            checked={agreed}
            ...
            style={{
              width: 18, height: 18, borderRadius: 4, accentColor: tokens.colors.accent,
              marginTop: 1, flexShrink: 0,
            }}
          />
          <span>
            I agree to the{" "}
            <span style={{ color: tokens.colors.accent, fontWeight: 500 }}>Terms of Service</span>
            {" "}and{" "}
            <span style={{ color: tokens.colors.accent, fontWeight: 500 }}>Privacy Policy</span>
          </span>
        </label>

        {/* Sign Up */}
        <Button variant="primary" onClick={onSignUp} disabled={!valid}>
          Create Account
        </Button>
      </div>
