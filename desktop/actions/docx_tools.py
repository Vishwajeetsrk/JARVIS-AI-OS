...
def _docx_result_path(source_path: Path | None, action: str, output_path: str | None, title: str) -> Path:
    if output_path:
        fallback = title or (source_path.stem if source_path else "JARVIS_AI_Document")
    ...
    return _resolve_output_path(None, title=title, ext=".docx", fallback_name="JARVIS_AI_Document")
