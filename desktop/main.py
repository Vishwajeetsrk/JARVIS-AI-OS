...
STARTUP_LOG     = Path(os.environ.get("LOCALAPPDATA", str(BASE_DIR))) / "JARVISAI" / "startup.log"
...
def _ensure_desktop_shortcut() -> None:
    ...
    try:
        ...
        shortcut_path = desktop_dir / "JARVIS AI - Lite.lnk"
        ...
        ps1_script = "\n".join([
            ...
            "$Shortcut.Description = 'Launch JARVIS AI - Lite'",
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are JARVIS AI - Lite, a calm, direct, and professional AI assistant. "
...
def _gemini_text_reply(prompt: str) -> str:
    ...
    system_prompt = (
        "You are JARVIS AI - Lite, a concise, helpful desktop assistant. "
...
def _update_memory_async(user_text: str, JARVIS_text: str) -> None:
    ...
    # Mirror this turn into the web brain (threads + shared memory) when linked.
    if make_cloud_sync is not None:
        try:
            cloud = make_cloud_sync()
            if cloud is not None and cloud.login():
                cloud.sync_turn(user_text, JARVIS_text)
        except Exception as e:
            print(f"[Cloud] ⚠️ sync failed: {e}")
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "JARVIS automatically infers the best visual style from the topic, searches for a matching online template when available, "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "theme": {
                    "type": "STRING",
                    "description": "Optional presentation theme or visual direction such as neon, corporate, luxury, academic, sunset, or creative. If omitted, JARVIS infers the best style automatically."
                },
                "outline": {
                    "type": "STRING",
...
class JARVISLive:
    ...
    def __init__(self, ui: JARVISUI, dashboard=None, dashboard_started: bool = False, enable_dashboard: bool = True):
        self.ui             = ui
        ...
        # Optional bridge to the Jarvis web brain (Supabase). Best-effort only:
        # if config/jarvis_cloud.json is missing, self._cloud stays None and
        # the desktop runs standalone.
        self._cloud = make_cloud_sync() if make_cloud_sync else None
        if self._cloud is not None:
            threading.Thread(target=self._cloud.startup, daemon=True).start()
    ...
    def _should_announce_idle(self) -> bool:
        if self.ui.muted:
            return False
        if self._cloud is not None and not self._cloud.voice_enabled:
            return False
    ...
    def _on_text_command(self, text: str, source: str = "local"):
        ...
        try:
            ...
            if routed_text_home != text:
                print(f"[JARVIS] Redirection: '{text}' -> '{routed_text_home}'")
                text = routed_text_home
        except Exception as e:
            print(f"[JARVIS] Redirection error: {e}")
        ...
        if _looks_like_screen_request(text):
            try:
                self.ui.update_task_workspace(
                    ...
                    output="JARVIS is inspecting the screen for what you asked about.",
                    percent=40,
                )
            except Exception:
                pass
    ...
    def _sync_turn_cloud(self, user_text: str, assistant_text: str):
        if self._cloud is None:
            return
        def _run():
            try:
                if self._cloud.login():
                    self._cloud.sync_turn(user_text, assistant_text)
            except Exception:
                pass
    ...
    def _fallback_reply(self, text: str, memory_ctx: str = ""):
        try:
            ...
            try:
                self.ui.update_task_workspace(
                    status="Thinking",
                    output="JARVIS is drafting a direct reply.",
                    percent=35,
                )
            except Exception:
                pass
            ...
            if gemini_first:
                try:
                    ...
                    print(f"[JARVIS] ⚠️ Gemini fallback failed: {e}")
            ...
            if not reply:
                try:
                    ...
                    print(f"[JARVIS] ⚠️ OpenRouter fallback failed: {e}")
            ...
            self._sync_turn_cloud(text, reply)
            try:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts.append(
            "Wake-word mode: if the microphone is muted, still listen for the words 'JARVIS', 'hey', 'hi', and 'hello'. "
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            ...
            if self._phone_active:
                return
            if not JARVIS_speaking and (not self.ui.muted or getattr(self.ui, "_wakeword_listening", False)):
                data = indata.tobytes()
                        ...
                        if sc.input_transcription and sc.input_transcription.text:
                            ...
                            if txt:
                                try:
                                    ...
                                    pass
                                in_buf.append(txt)
                                if self.ui.muted and _wakeword_detected(txt) and (self._cloud is None or self._cloud.wake_word_enabled):
                                    try:
    ...
    async def run(self):
        ...
        while True:
                ...
                try:
                    async with asyncio.TaskGroup() as tg:
                        ...
                        try:
                            ...
                            pass
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: JARVIS AI online.")
            ...
            self.set_speaking(False)
            self.ui.set_state("LISTENING")
            print("[JARVIS] 🔄 Reconnecting in 5s...")
            await asyncio.sleep(5)

def main():
    _startup_log("main entered")
    _ensure_desktop_shortcut()
    ui = JARVISUI(str(BASE_DIR / "assets" / "JARVIS_Lite_Logo.png"), show_immediately=True)
    dashboard = None
    ...
    if DashboardServer is not None and not dashboard_enabled:
        ...
        try:
            ui.write_log("SYS: Mobile Connect is already running in another JARVIS instance.")
        except Exception:
            pass
    ...
    def runner():
        ...
        try:
            if plugin_manager is not None:
                JARVIS.plugin_manager = plugin_manager
                plugin_manager.register_JARVIS(JARVIS)
                # allow plugins to run a startup hook
                try:
                    plugin_manager.dispatch("on_startup", JARVIS)
                except Exception:
                    pass
        except Exception:
            pass
