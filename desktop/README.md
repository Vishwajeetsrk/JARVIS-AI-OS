<p align="center">
  <img src="assets/JARVIS_Lite_Logo.png" alt="JARVIS AI - Lite" width="260" />
</p>

<h1 align="center">JARVIS AI - Lite</h1>
...
JARVIS AI - Lite is an open-source desktop assistant designed for Windows power users. It unites voice and text input with intelligent automation, productivity workflows, document generation, and adaptive screen-aware actions.
...
- Built-in website and workspace creation through JARVIS's local workspace generator
...
JARVIS AI - Lite is built on a layered desktop architecture that separates UI, AI session management, and tool execution.

- `main.py` initializes the application, launches the UI, and manages the AI runtime.
- `JARVISLive` owns the live AI session, audio queues, and command routing.
...
```powershell
git clone https://github.com/titechprabhasolutions/JARVIS-AI---Lite.git
cd "JARVIS AI - Lite"
```
...
You can extend JARVIS AI with lightweight Python plugins placed in the `plugins/` folder. Plugins are simple `*.py` files that export one or more hook functions:

- `on_JARVIS_created(JARVIS)` — called when the `JARVISLive` instance is created
- `on_startup(JARVIS)` — called once during app startup after plugins are registered
- `on_text_command(text, source, JARVIS=None)` — called for each incoming text command; return `True` to mark the command handled and stop further processing
...
- Tightened the developer workflow so JARVIS uses the local workspace generator for websites and keeps coding tasks in the main app flow.
...
Please preserve attribution and keep credentials secure when building on top of JARVIS AI - Lite.
