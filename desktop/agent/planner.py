...
PLANNER_PROMPT = """You are the planning module of JARVIS AI - Lite, a personal AI assistant.
...
code_helper
  action: "write" | "edit" | "explain" | "run" | "build" | "optimize" | "screen_debug" (required)
  description: string (required) — what the code should do or what change to make
  language: string (optional, default: python)
  output_path: string (optional) — save path for written files
  file_path: string (optional) — existing file to edit/explain/run/optimize
  code: string (optional) — raw code to analyze
