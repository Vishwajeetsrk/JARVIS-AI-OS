...
export function Terminal({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "mx-auto w-full max-w-xl px-2 font-mono text-xs",
        className,
      )}
    >
      <div className="overflow-hidden rounded-2xl border border-cyan-500/30 bg-neutral-950/95 shadow-2xl backdrop-blur-xl">
        {/* Title Bar */}
        <div className="flex items-center justify-between border-b border-white/10 bg-neutral-900/80 px-4 py-2.5">
          ...
          <div className="w-12 text-right text-[10px] text-cyan-400">● LIVE</div>
        </div>

        {/* Terminal Screen */}
        <div
          ref={contentRef}
          className="h-72 overflow-y-auto p-4 font-mono leading-relaxed"
          style={{
            background: "radial-gradient(ellipse at top left, rgba(0, 229, 255, 0.05) 0%, #030712 100%)",
          }}
        >
          {lines.map((line, i) => (
            <div key={i} className="mb-1 whitespace-pre-wrap">
              {line.type === "command" ? (
                ...
                <span className="text-emerald-300/90 pl-2 border-l border-emerald-500/30 block my-0.5">
                  {line.content}
                </span>
              )}
            </div>
          ...
          {phase === "typing" && (
            <div className="whitespace-pre-wrap">
              ...
              <span className="ml-1 inline-block h-3.5 w-2 bg-cyan-400 align-middle shadow-[0_0_8px_#00e5ff]" />
            </div>
          )}
          ...
          {(phase === "done" || phase === "pausing" || phase === "outputting") && (
            <div className="whitespace-pre-wrap">
              ...
              <span
                className={cn(
                  "inline-block h-3.5 w-2 bg-cyan-400 align-middle transition-opacity duration-100 shadow-[0_0_8px_#00e5ff]",
